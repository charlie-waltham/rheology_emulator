...
class TorchDataManager:
    def __init__(
        self, file_path, arguments=None, zarr_fmt="fmt1", difference_labels=False
    ):
    ...
    def _print_summary(self):
        ...
        print(
            f"Dataset sizes: Train={len(self.train.dataset)}, Val={len(self.val.dataset)}, Test={len(self.test.dataset)}"
        )
    ...
    def _load_zarr_1(self):
        ...
        mask_features = siconc_mask.expand_dims(
            {"feature": pairs["features"].feature}, axis=1
        )
            ...
            for ilabel, label in enumerate(self.train_labels):
                if label in self.train_features:
                    new_label_values = (
                        ...
                        - features.sel(feature=label).values
                    )
        ...
        for pair in range(features.shape[0]):
            fl = (features[pair].values.T, labels[pair].values.T)
            pd_pairs.append(fl)
    ...
    def _scale_features(self, pairs):
        ...
        scaler.fit(
            pairs[0][0]
        )  # TODO: This assumes first feature is representative, so need to think about this
