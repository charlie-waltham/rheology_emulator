...
def make_pairs(filelist, features, labels, stack_pairs=True, apply_siconc_mask=True):
    """
    ...
    for input_path in filelist:
        ...
        features_arr = (
            xr.concat(
                [ds[var].isel(time_counter=slice(0, -1)) for var in features],
                dim="feature",
            )
            .transpose("time_counter", "feature", "y", "x")
            .data
        )  # Dask array

        labels_arr = (
            xr.concat(
                [ds[var].isel(time_counter=slice(1, None)) for var in labels],
                dim="label",
            )
            .transpose("time_counter", "label", "y", "x")
            .data
        )  # Dask array
    ...
    if stack_pairs:
        # Stack the pair dimension into the spatial dimensions to create a simpler dataset
        pairs_stacked = paired_ds.stack(z=("pair", "y", "x"))

        if apply_siconc_mask:
            # Mask out pairs where siconc > 0
            siconc = pairs_stacked["features"].sel(feature="siconc")
            # 1. Get siconc after stacking (dims: z)
            siconc = pairs_stacked["features"].sel(feature="siconc")

            # 2. Create mask where siconc > 0 (dims: z)
            sic_mask = siconc > 0

            # 3. Broadcast mask to features and labels
            sic_mask_features = sic_mask.expand_dims(
                {"feature": pairs_stacked["features"].feature}
            )
            sic_mask_labels = sic_mask.expand_dims(
                {"label": pairs_stacked["labels"].label}
            )

            # 4. Apply mask
            pairs_stacked["features"] = pairs_stacked["features"].where(
                sic_mask_features
            )
            pairs_stacked["labels"] = pairs_stacked["labels"].where(sic_mask_labels)

            # 5. Drop NaNs along z for the whole dataset (keeps features and labels aligned)
            pairs_stacked = pairs_stacked.dropna(dim="z", how="any")
            pairs_stacked = pairs_stacked.reset_index("z")

            paired_ds = pairs_stacked
