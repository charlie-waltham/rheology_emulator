...
def make_pairs(filelist, features, labels, stack_pairs=True, apply_siconc_mask=True):
    ...
    if stack_pairs:
        ...
        if apply_siconc_mask:
            ...
            sic_mask_features = sic_mask.expand_dims({'feature': pairs_stacked['features'].feature})
            sic_mask_labels = sic_mask.expand_dims({'label': pairs_stacked['labels'].label})

            # 4. Apply mask
            pairs_stacked['features'] = pairs_stacked['features'].where(sic_mask_features)
