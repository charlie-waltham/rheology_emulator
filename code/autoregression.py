...
from src.inference import RheologyModel


def autoregression(model_path, dataset_path):
    plt.style.use("seaborn-v0_8")

    t = time.time()
    dataset: xr.Dataset = xr.open_zarr(dataset_path)
    dataset = dataset.sel(
        {
            "feature": [
                "siconc",
...
def main():
    if len(sys.argv) > 2:
        ...
        print("Usage: python autoregression.py <model> <dataset>")
        exit(0)

    autoregression(model, dataset)
